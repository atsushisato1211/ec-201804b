...
public class ItemRepository {
	...
	public List<Item> findAllBySortAndNotDeleted(String itemSort,String sortOption) {
		String sql = "select id,name,description,producingArea,season,price,imagePath,deleted from " + TABLE_NAME + " where deleted=false order by "+itemSort+" "+sortOption+" limit 15;";
	...
	public List<Item> findAllByNotDeleted() {
		String sql = "select id,name,description,producingArea,season,price,imagePath,deleted from " + TABLE_NAME + " where deleted=false order by id limit 15";
		List<Item> itemList = template.query(sql, ITEM_ROWMAPPEP);
		return itemList;
	}
	public List<Item> findAllBypageNumNotDeleted(Integer pageNum) {
		String sql = "select id,name,description,producingArea,season,price,imagePath,deleted from " + TABLE_NAME + " where deleted=false order by id limit "+(15)+" OFFSET "+(pageNum*15-15);
		List<Item> itemList = template.query(sql, ITEM_ROWMAPPEP);
		return itemList;
	}
	public List<Item> findAllBypageNumNotDeletedSort(Integer pageNum,String itemSort,String sortOption) {
		String sql = "select id,name,description,producingArea,season,price,imagePath,deleted from " + TABLE_NAME + " where deleted=false order by "+itemSort+" "+sortOption+" limit "+(15)+" OFFSET "+(pageNum*15-15);
		List<Item> itemList = template.query(sql, ITEM_ROWMAPPEP);
		return itemList;
	}
